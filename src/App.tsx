...
import { doc, setDoc, updateDoc, deleteDoc, collection, onSnapshot, writeBatch, getDocFromServer } from 'firebase/firestore';
import { auth, db, handleFirestoreError, OperationType, cleanObject } from './firebase';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubConfig = onSnapshot(doc(db, 'config', 'shop'), (snap) => {
      if (!snap.exists()) {
        try {
  ...
  const handleLogout = async () => {
    try {
      ...
      setProducts(getStoredData<Product[]>('lark_pos_products', INITIAL_PRODUCTS));
      setCustomers(getStoredData<Customer[]>('lark_pos_customers', INITIAL_CUSTOMERS));
  ...
  const handleAddOrder = async (newOrder: Order) => {
    if (user) {
      ...
      products.forEach((p) => {
        const containsOrderItem = newOrder.items.some((it) => it.productId === p.id);
        if (!containsOrderItem) return;

        const updatedVariants = p.variants.map((v) => {
          const matchingCheckoutItem = newOrder.items.find((it) => it.variantId === v.id);
          if (!matchingCheckoutItem) return v;

          const nextAvailable = Math.max(0, v.available - matchingCheckoutItem.quantity);
        ...
        const totalAvailable = updatedVariants.reduce((s, v) => s + v.available, 0);
        const totalShipping = updatedVariants.reduce((s, v) => s + v.shipping, 0);
      ...
      if (newOrder.customer) {
        const matchedCust = customers.find((c) => c.id === newOrder.customer?.id);
        if (matchedCust) {
          const updatedCust = {
            ...matchedCust,
          ...
          batch.set(doc(db, 'customers', matchedCust.id), cleanObject(updatedCust));
        }
      }
      ...
      try {
        ...
        handleFirestoreError(err, OperationType.WRITE, `orders/batch/${newOrder.id}`);
      }
  ...
  const handleUpdateOrderStatus = async (orderId: string, status: Order['status']) => {
    ...
    if (user) {
      ...
      if (status === 'canceled' && targetOrder.status !== 'canceled') {
        products.forEach((p) => {
          const containsOrderItem = targetOrder.items.some((it) => it.productId === p.id);
          if (!containsOrderItem) return;

          const updatedVariants = p.variants.map((v) => {
            const matchingCheckoutItem = targetOrder.items.find((it) => it.variantId === v.id);
            if (!matchingCheckoutItem) return v;

            const nextAvailable = v.available + matchingCheckoutItem.quantity;
            const nextShipping = Math.max(0, v.shipping - matchingCheckoutItem.quantity);
            return {
              ...v,
              available: nextAvailable,
              shipping: nextShipping
        ...
        if (targetOrder.customer) {
          const matchedCust = customers.find((c) => c.id === targetOrder.customer?.id);
          if (matchedCust) {
            const updatedCust = {
              ...matchedCust,
              totalSpent: Math.max(0, matchedCust.totalSpent - targetOrder.finalAmount),
      ...
      if (status === 'completed' && targetOrder.status !== 'completed') {
        products.forEach((p) => {
          ...
          const updatedVariants = p.variants.map((v) => {
            ...
            const nextStock = Math.max(0, v.stock - matchingCheckoutItem.quantity);
            const nextShipping = Math.max(0, v.shipping - matchingCheckoutItem.quantity);
            return {
              ...v,
              stock: nextStock,
          ...
          setCustomers((prevCustomers) =>
            prevCustomers.map((c) => {
              if (c.id === targetOrder.customer?.id) {
                return {
                  ...c,
                  totalSpent: Math.max(0, c.totalSpent - targetOrder.finalAmount),
  ...
  const handleAddCustomer = async (newCustomer: Customer) => {
    if (user) {
      try {
        await setDoc(doc(db, 'customers', newCustomer.id), cleanObject(newCustomer));
      } catch (err) {
  ...
  const handleAddProduct = async (newProduct: Product) => {
    if (user) {
      try {
        await setDoc(doc(db, 'products', newProduct.id), cleanObject(newProduct));
      } catch (err) {
  ...
  const handleUpdateProductStatus = async (productId: string, isActive: boolean) => {
    if (user) {
      try {
        // Since we update a single key, updateDoc is clean
  ...
  return (
    <div className="h-screen w-screen bg-transparent p-[5px]">
      <div className="flex h-full w-full overflow-hidden bg-white/20 backdrop-blur-md rounded-[15px] shadow-2xl border border-white/30 font-sans" id="pos-application-root">
        ...
        <main className="flex-1 overflow-y-auto min-h-0 bg-transparent">
          ...
          {['not_implemented_yet'].includes(currentTab) && (
            <div className="h-full flex flex-col items-center justify-center text-slate-400 bg-slate-50/50">
              ...
              <h2 className="text-lg font-bold text-slate-800">Tính năng đang phát triển</h2>
              <p className="text-sm">Mục <b>{currentTab}</b> sẽ sớm được cập nhật trong phiên bản tiếp theo.</p>
              <button 
