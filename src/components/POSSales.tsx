...
export default function POSSales({
  ...
  // Tabs State
  const [orders, setOrders] = useState<OrderState[]>([DEFAULT_ORDER_STATE('order-1', 'Đơn mới (F6)')]);
  ...
  const updateActiveOrder = (updates: Partial<OrderState>) => {
    setOrders(prev => prev.map(o => o.id === activeOrderId ? { ...o, ...updates } : o));
  };
  ...
  const handleAddVariantToCart = (prod: Product, v: Variant) => {
    ...
    if (existing) {
      ...
      updateActiveOrder({ cart: [...activeOrder.cart, { product: prod, variant: v, quantity: 1 }] });
    }
  };

  const removeCartItem = (variantId: string) => {
    updateActiveOrder({ cart: activeOrder.cart.filter(it => it.variant.id !== variantId) });
  };
  ...
  const totalAmount = Math.max(0, subTotal - discountVal + activeOrder.shippingFee + activeOrder.surcharge);
  ...
  const applyPromoCode = (code: string) => {
    ...
    let promos = [];
    if (savedPromos) {
      try { promos = JSON.parse(savedPromos); } catch (e) {}
    }
    ...
    if (now < start) {
      setAppliedPromoError(`Chương trình bắt đầu từ ${start.toLocaleDateString('vi-VN')}!`);
      return;
    }
    if (now > end) {
    ...
    if (subTotal < promo.minOrderValue) {
      setAppliedPromoError(`Mua tối thiểu ${promo.minOrderValue.toLocaleString()}đ để dùng mã này!`);
      return;
    }
    
    if (promo.targetCustomerType && promo.targetCustomerType !== 'all') {
      const selectedCust = activeOrder.selectedCustomerId ? customers.find(c => c.id === activeOrder.selectedCustomerId) : null;
      const custType = selectedCust ? selectedCust.type : 'regular';
      if (custType !== promo.targetCustomerType) {
        setAppliedPromoError(`Voucher áp dụng riêng cho phân khúc khách ${promo.targetCustomerType === 'vip' ? 'VIP' : 'Sỉ'}.`);
        return;
      }
    }
    ...
    if (promo.type === 'percentage') {
      ...
      setAppliedPromoSuccess(`Áp dụng ${promo.code}: Giảm ${promo.value}% (-${discVal.toLocaleString()}đ)`);
      ...
      setAppliedPromoSuccess(`Áp dụng ${promo.code}: Giảm trực tiếp -${discVal.toLocaleString()}đ`);
      ...
      setAppliedPromoSuccess(`Áp dụng ${promo.code}: Mua X tặng Y (Tặng thêm ${promo.value} SP).`);
    }
    ...
    if (e.key === 'Enter' && searchTerm) {
      ...
      for (const p of products) {
        for (const v of p.variants) {
          if (v.sku.toLowerCase() === termToLower || p.sku.toLowerCase() === termToLower) {
      ...
      alert('Không tìm thấy sản phẩm có mã barcode/SKU này!');
    }
  };
        ...
        <div className="flex items-center gap-1 overflow-x-auto scrollbar-none flex-1">
          {orders.map(o => (
            <div
              ...
              className={`flex items-center gap-2 px-4 h-9 rounded-t-lg border-x border-t transition-all cursor-pointer text-xs font-bold whitespace-nowrap ${
                activeOrderId === o.id 
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex-1">
                ...
                <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2 text-slate-400">
                   ...
                   <button
                      onClick={() => setIsBarcodeScannerMode(!isBarcodeScannerMode)}
                   ...
                   >
                     <Scan size={16} />
                     <span className="text-[10px] font-bold">Quét mã vạch</span>
                   </button>
                  ...
                  {searchTerm && filteredProducts.length > 0 && (
                    ...
                    >
                      {filteredProducts.map(p => (
                        <div key={p.id} className="border-b border-slate-50 last:border-none">
                          {p.variants.map(v => (
                            <div 
                              key={v.id} 
                              onClick={() => {
                            ...
                            >
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-slate-500">{v.name} - SKU: {v.sku}</p>
                                </div>
                              </div>
                              <div className="text-right">
                                <p className="font-bold text-blue-600">{v.price.toLocaleString()}đ</p>
                                <p className="text-[10px] text-slate-400">Còn: {v.available}</p>
                              </div>
                            </div>
                          ))}
                        </div>
                  ...
                  <table className="w-full text-xs text-left">
                    ...
                    <tbody className="bg-white">
                      {activeOrder.cart.map(item => (
                        <tr key={item.variant.id} className="border-b border-slate-100">
                          <td className="py-3 px-3">
                            <p className="font-bold">{item.product.name}</p>
                            <p className="text-slate-400 text-[10px]">{item.variant.name}</p>
                          </td>
                          <td className="py-3 px-3">
                            <div className="flex items-center gap-2">
                              <button onClick={() => updateCartQty(item.variant.id, -1)} className="p-1 hover:bg-slate-100 rounded"><Minus size={12}/></button>
                              <span className="w-6 text-center font-bold">{item.quantity}</span>
                              <button onClick={() => updateCartQty(item.variant.id, 1)} className="p-1 hover:bg-slate-100 rounded"><Plus size={12}/></button>
                            </div>
                          </td>
                          <td className="py-3 px-3 font-medium">{item.variant.price.toLocaleString()}đ</td>
                          <td className="py-3 px-3 font-bold text-right">{(item.variant.price * item.quantity).toLocaleString()}đ</td>
                          <td className="py-3 px-3 text-center">
                            <button onClick={() => removeCartItem(item.variant.id)} className="text-slate-300 hover:text-red-500 transition">
                              <Trash2 size={16} />
                            </button>
                          </td>
                        </tr>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="glass-card rounded-lg p-4 space-y-4">
              <div className="flex items-center justify-between">
                <h4 className="font-bold text-sm">Giá trị đơn hàng</h4>
                <MoreVertical size={16} className="text-slate-400" />
              </div>
              <div className="space-y-3">
                <div className="flex items-center gap-4 text-[11px] font-medium">
                  <div className="flex items-center gap-2">
                    <div className={`w-4 h-4 rounded border ${activeOrder.freeShipping ? 'bg-blue-600 border-blue-600' : 'border-slate-300 bg-white'}`} 
                       onClick={() => updateActiveOrder({ freeShipping: !activeOrder.freeShipping })} />
                    <span>Miễn phí giao hàng</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className={`w-4 h-4 rounded border ${activeOrder.onlyChargeIfReturn ? 'bg-blue-600 border-blue-600' : 'border-slate-300 bg-white'}`}
                       onClick={() => updateActiveOrder({ onlyChargeIfReturn: !activeOrder.onlyChargeIfReturn })} />
                ...
                <div className="space-y-2">
                  <div className="flex items-start justify-between text-xs">
                    <span className="text-slate-500 py-1 border-transparent border">Giảm giá</span>
                    <div className="flex flex-col items-end gap-1.5">
                      <div className="flex bg-slate-100 rounded-lg overflow-hidden w-32 border border-slate-200">
                        <input 
                          type="number" 
                          className="w-full bg-transparent border-none text-right px-2 py-1 text-xs font-bold outline-none" 
                          value={activeOrder.discountValue}
                          onChange={(e) => updateActiveOrder({ discountValue: Number(e.target.value) })}
                        />
                        <button 
                          className="bg-slate-200 px-2 text-[10px] font-bold"
                          onClick={() => updateActiveOrder({ discountType: activeOrder.discountType === 'cash' ? 'percent' : 'cash' })}
                        >
                          {activeOrder.discountType === 'cash' ? 'đ' : '%'}
                        </button>
                      </div>
                      <div className="flex gap-1">
                        <button onClick={() => updateActiveOrder({ discountType: 'percent', discountValue: 5 })} className="text-[9px] px-1.5 py-0.5 rounded bg-blue-50 text-blue-600 font-bold border border-blue-100 transition hover:bg-blue-100">5%</button>
                        <button onClick={() => updateActiveOrder({ discountType: 'percent', discountValue: 10 })} className="text-[9px] px-1.5 py-0.5 rounded bg-blue-50 text-blue-600 font-bold border border-blue-100 transition hover:bg-blue-100">10%</button>
                        <button onClick={() => updateActiveOrder({ discountType: 'percent', discountValue: 15 })} className="text-[9px] px-1.5 py-0.5 rounded bg-blue-50 text-blue-600 font-bold border border-blue-100 transition hover:bg-blue-100">15%</button>
                        <button onClick={() => updateActiveOrder({ discountType: 'cash', discountValue: 50000 })} className="text-[9px] px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-600 font-bold border border-emerald-100 transition hover:bg-emerald-100">-50k</button>
                      </div>
                    </div>
                  </div>

                  {/* Dynamic Promotions Voucher Selector */}
                  <div className="border-t border-dashed border-slate-200 pt-2 pb-1 space-y-1.5 text-[11px]">
                    ...
                    <div className="flex bg-slate-100 rounded-lg overflow-hidden border border-slate-200">
                      <input 
                        ...
                        className="w-full bg-transparent border-none px-2 py-1 text-xs font-bold outline-none uppercase placeholder:text-slate-400 placeholder:normal-case placeholder:font-normal" 
                      ...
                      <button 
                        type="button" 
                        className="bg-indigo-600 text-white px-3 text-[10px] font-extrabold hover:bg-indigo-700 transition"
                    ...
                    {appliedPromoSuccess && (
                      <div className="text-[9.5px] text-emerald-600 font-black leading-snug bg-emerald-50 border border-emerald-100 rounded-md p-1.5 flex items-center justify-between">
                        ...
                        <button 
                          className="hover:text-red-500 font-extrabold text-[11px] p-0.5 ml-1"
                          onClick={() => {
                            setAppliedPromoSuccess('');
                            setTypedPromoCode('');
                    ...
                    {showPromoList && (
                      <div className="bg-slate-50 rounded-lg border border-slate-200 p-2 space-y-2 max-h-36 overflow-y-auto scrollbar-thin">
                        <span className="text-[9px] font-bold uppercase tracking-wider text-slate-400 block pb-1 border-b border-slate-200">
                              ...
                              <div 
                                ...
                                onClick={() => {
                                  if (isEligible) {
                                    ...
                                    setAppliedPromoError(`Đơn hàng cần từ ${p.minOrderValue.toLocaleString()}đ để dùng mã "${p.code}"`);
                                  }
                                }}
                              >
                                <div className="flex items-center justify-between font-black">
                                  ...
                                  <span className="text-slate-750">
                                    {p.type === 'percentage' ? `-${p.value}%` : `-${Math.round(p.value / 1000)}k`}
                                  </span>
                                </div>
                                <p className="text-[9px] text-slate-500 font-medium leading-tight mt-1 line-clamp-2">
                                  {p.name}
                                </p>
                                <div className="text-[8px] text-slate-400 flex justify-between mt-1 pt-1 border-t border-slate-100 font-bold">
                                  <span>Đơn từ: {p.minOrderValue.toLocaleString()}đ</span>
                                  {!isEligible && (
                                    <span className="text-amber-600">Thiếu: {(p.minOrderValue - subTotal).toLocaleString()}đ</span>
                                  )}
                                </div>
                              </div>
                            );
                  ...
                  <div className="flex items-center justify-between text-xs pt-1">
                    ...
                    <div className="flex bg-slate-100 rounded-lg overflow-hidden w-32 border border-slate-200">
                      <input 
                        ...
                        onChange={(e) => updateActiveOrder({ shippingFee: Number(e.target.value) })}
                      />
                      <span className="bg-slate-200 px-2 flex items-center text-[10px] font-bold">đ</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <div className="flex bg-slate-100 rounded-lg overflow-hidden w-32 border border-slate-200">
                      <input 
                        ...
                        onChange={(e) => updateActiveOrder({ surcharge: Number(e.target.value) })}
                      />
                      <span className="bg-slate-200 px-2 flex items-center text-[10px] font-bold">đ</span>
                    </div>
                  </div>
                ...
                <div className="pt-2 border-t border-slate-100 space-y-1.5">
                  ...
                  <div className="flex justify-between text-xs">
                    <span>Giảm giá</span>
                    <span className="font-bold text-red-500">{discountVal.toLocaleString()} đ</span>
                  </div>
                  <div className="flex justify-between text-xs pt-1 border-t border-dashed border-slate-200">
                    <span className="font-bold">Tiền cần thu</span>
                    <span className="font-bold text-green-600">{totalAmount.toLocaleString()} đ</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <div className="flex bg-slate-100 rounded-lg overflow-hidden w-40 border border-slate-200">
                    <input 
                      type="number" 
                      className="w-full bg-transparent border-none text-right px-2 py-1.5 text-xs font-bold outline-none" 
                      value={activeOrder.paymentAmount || totalAmount}
                      onChange={(e) => updateActiveOrder({ paymentAmount: Number(e.target.value) })}
                    />
                    <span className="bg-slate-200 px-2 flex items-center text-[10px] font-bold">đ</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-1.5 pt-2">
                   {[
                     ...
                     <button 
                       key={m.id}
                       onClick={() => updateActiveOrder({ paymentMethod: m.id as any })}
            ...
            {/* Section: Ghi chú */}
            <div className="glass-card rounded-lg p-4 space-y-4">
              <h4 className="font-bold text-sm">Ghi chú</h4>
              <div className="flex bg-slate-100 p-0.5 rounded-lg">
                <button 
                   className="flex-1 py-1.5 text-[11px] font-bold bg-white shadow-sm text-blue-600 rounded-md"
                >
                  Nội bộ
                </button>
                <button 
                   className="flex-1 py-1.5 text-[11px] font-bold text-slate-500 rounded-md"
                >
                  Để in
                </button>
              </div>
              <div className="space-y-3">
                <textarea 
                  className="w-full h-24 bg-slate-100 rounded-lg p-3 text-xs font-medium border-none outline-none resize-none placeholder:text-slate-400"
                  placeholder="Viết ghi chú hoặc /shortcut để ghi chú nhanh"
                  value={activeOrder.internalNotes}
                  onChange={(e) => updateActiveOrder({ internalNotes: e.target.value })}
                />
        ...
        <div className="flex-[4] flex flex-col gap-3 min-w-[320px]">
          ...
          <div className="glass-card rounded-lg p-4 space-y-4">
            <div className="flex items-center justify-between cursor-pointer">
              <h4 className="font-bold text-sm">Thông tin</h4>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <div className="relative w-44">
                  <select 
                    className="w-full bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-bold appearance-none pr-8"
                    value={activeOrder.staffHandling}
                    onChange={(e) => updateActiveOrder({ staffHandling: e.target.value })}
                  >
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <div className="relative w-44">
                  <select className="w-full bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-bold appearance-none pr-8 italic text-slate-400">
              ...
              <button className="px-3 py-1.5 rounded-lg bg-slate-100 text-xs font-bold border border-slate-200">
                Thêm thẻ
              </button>
            </div>
          </div>
          ...
          <div className="glass-card rounded-lg p-4 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <div className="relative group">
                  <select className="bg-slate-100 px-2 py-1 rounded-lg text-[10px] font-bold appearance-none pr-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              <input 
                ...
                className="col-span-1 bg-slate-100 px-3 py-2 rounded-lg text-xs font-medium border-none outline-none" 
                value={activeOrder.customerName}
                onChange={(e) => updateActiveOrder({ customerName: e.target.value })}
              />
              <input 
                type="text" 
                placeholder="SĐT" 
                ...
                onChange={(e) => updateActiveOrder({ customerPhone: e.target.value })}
              />
              <input 
                type="text" 
                ...
                onChange={(e) => updateActiveOrder({ customerEmail: e.target.value })}
              />
              <div className="col-span-1 relative">
                <input 
                  type="text" 
                  placeholder="Ngày sinh" 
                  className="w-full bg-slate-100 px-3 py-2 rounded-lg text-xs font-medium border-none outline-none" 
                  value={activeOrder.customerBirthday}
                  onChange={(e) => updateActiveOrder({ customerBirthday: e.target.value })}
                />
          ...
          <div className="glass-card rounded-lg p-4 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="relative group">
                <select className="bg-slate-100 px-3 py-1.5 rounded-lg text-xs font-bold appearance-none pr-8">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-500 whitespace-nowrap">Dự kiến nhận hàng</span>
                <div className="relative w-44">
                  <input 
                    type="text" 
                    ...
                    onChange={(e) => updateActiveOrder({ expectedDeliveryDate: e.target.value })}
                  />
              ...
              <div className="grid grid-cols-2 gap-2">
                <input 
                  ...
                  onChange={(e) => updateActiveOrder({ recipientName: e.target.value })}
                />
                <input 
                  type="text" 
                  ...
                  onChange={(e) => updateActiveOrder({ recipientPhone: e.target.value })}
                />
              </div>
              <input 
                type="text" 
                ...
                onChange={(e) => updateActiveOrder({ recipientAddress: e.target.value })}
              />
              <div className="relative w-full">
                <select className="w-full bg-slate-100 px-4 py-2 rounded-lg text-xs font-bold appearance-none pr-10 text-slate-400">
                  <option>Chọn địa chỉ</option>
                </select>
          ...
          <div className="glass-card rounded-lg p-4 space-y-4">
             <div className="flex items-center justify-between">
                ...
                <div className="text-[10px] text-slate-400 font-bold px-2 py-1 rounded bg-slate-100 border border-slate-200">
                  Đơn vị VC: <span>Y</span>
                </div>
             </div>
             <p className="text-[11px] text-slate-400 italic font-medium">Thông tin cấu hình vận chuyển...</p>
          </div>
      ...
      <div className="h-16 bg-white border-t border-slate-200 px-6 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-8">
           <div className="flex flex-col">
             <span className="text-xs text-slate-400 font-medium">Tiền cần thu:</span>
             <span className="text-lg font-black text-slate-800">{totalAmount.toLocaleString()} <span className="text-xs">đ</span></span>
           </div>
           <div className="flex flex-col">
             <span className="text-xs text-slate-400 font-medium">COD:</span>
             <span className="text-lg font-black text-red-500">{totalAmount.toLocaleString()} <span className="text-xs">đ</span></span>
           </div>
        </div>

        <div className="flex items-center gap-3">
           <button 
             className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-slate-100 text-slate-600 text-xs font-extrabold border border-slate-200 hover:bg-slate-200 transition"
           >
             <Printer size={16} />
             <span>In (F4)</span>
           </button>
           <button 
             onClick={handleSave}
             className="flex items-center gap-2 px-6 py-2.5 rounded-lg bg-blue-600 text-white text-xs font-extrabold hover:bg-blue-700 transition shadow-lg shadow-blue-500/20"
           >
             <Save size={16} />
             <span>Lưu (F2)</span>
           </button>
