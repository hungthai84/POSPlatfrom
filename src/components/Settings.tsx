...
import { Settings, Shield, Store, RefreshCw, Mail, Phone, MapPin, Percent, HelpCircle, Save, CheckCircle2, Download } from 'lucide-react';
...
export default function SettingsView({
  ...
  const [shopLowStockThreshold, setShopLowStockThreshold] = useState<number>(config.lowStockThreshold ?? 5);
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => setShopLowStockThreshold(Number(e.target.value))}
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-xs text-slate-705 focus:outline-none focus:bg-white focus:border-blue-500 transition font-bold"
                    placeholder="5"
                  />
                </div>
        ...
        <div className="space-y-6 col-span-1">
          ...
          <div className="bg-white rounded-xl border border-slate-200/60 p-5 shadow-3xs space-y-4">
            <div className="flex items-center gap-1.5 text-blue-600 border-b border-slate-100 pb-3">
              <Download size={16} />
              <span className="font-extrabold text-slate-800 text-xs uppercase tracking-wide">Sao lưu toàn vẹn dữ liệu</span>
            </div>
            <p className="text-[11px] text-slate-450 leading-relaxed mt-1">
              Tải xuống toàn bộ cấu hình cửa hàng, sản phẩm, khách hàng và danh sách lịch sử giao dịch hiện tại dưới định dạng file <strong>JSON</strong> dự phòng.
            </p>
            <button
              onClick={handleBackup}
              type="button"
              className="w-full bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs py-2.5 rounded-lg flex items-center justify-center gap-1.5 transition shadow-sm"
            >
              <Download size={14} />
