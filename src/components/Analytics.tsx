...
export default function Analytics({
  ...
  const totalProfit = useMemo(() => {
    ...
    salesCompleted.forEach((o) => {
      o.items.forEach((item) => {
        ...
        const foundVar = foundProd?.variants.find(
          (v) => v.id === item.variantId,
        );
        const singleCogs =
          foundVar?.importPrice || Math.round(item.price * 0.5);
  ...
  const bestSellingProducts = useMemo(() => {
    const itemsMap: {
      [key: string]: { name: string; sku: string; qty: number; value: number };
    } = {};
    salesCompleted.forEach((o) => {
      o.items.forEach((it) => {
        if (!itemsMap[it.variantId]) {
          itemsMap[it.variantId] = {
            name:
              it.productName +
              " (" +
              (it.variantName.split(" - ")[1] || "Bản chuẩn") +
              ")",
            sku: it.sku,
            qty: 0,
            value: 0,
  ...
  const forecastData = useMemo(() => {
    ...
    for (let i = 90; i >= 0; i--) {
      ...
      const stockVal =
        i === 0
          ...
          : Math.max(selectedVariant.available, simulatedStock);
    ...
    const daysToDepletion =
      dailyVelocity > 0
        ? Math.ceil(selectedVariant.available / dailyVelocity)
        : 999;
    const depletionDate = new Date();
    depletionDate.setDate(
      depletionDate.getDate() + (dailyVelocity > 0 ? daysToDepletion : 90),
    );

    const depletionDateStr =
      dailyVelocity > 0
    ...
    const projectionSteps =
      dailyVelocity > 0 ? Math.min(30, daysToDepletion) : 30;
    ...
    >
      ...
      <div className="space-y-0.5">
        ...
        <p className="text-[14px] text-slate-450">
          Báo cáo doanh thu kinh doanh chi tiết phục vụ tối ưu hóa chi phí.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200/60 shadow-3xs">
          <span className="text-[11px] font-bold text-slate-400 block uppercase">
            Giá trị TB/Hóa đơn POS
          </span>
          <h3 className="text-xl font-black text-slate-800 mt-1">
            {formatCurrency(averageBasketValue)}
          </h3>
          <span className="text-[11px] text-slate-400 mt-1 block font-semibold">
            Tăng 2.5% so với quý trước
          </span>
        </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200/60 shadow-3xs">
          ...
          <h3 className="text-xl font-black text-slate-800 mt-1">
            {salesCompleted.length} Hóa đơn
          </h3>
          <span className="text-[11px] text-indigo-650 font-bold mt-1 block">
            Tỷ lệ hủy đơn cực thấp: 0%
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200/60 p-5 shadow-3xs space-y-4">
          <h4 className="font-bold text-slate-800 text-[16px]">
            Doanh thu bán hằng ngày (Tháng này)
          </h4>
        ...
        <div className="bg-white rounded-xl border border-slate-200/60 p-5 shadow-3xs space-y-4">
          <h4 className="font-bold text-slate-800 text-[16px]">
            Lượng tiêu thụ sản phẩm theo Ngành hàng
          </h4>
          <div className="h-64 w-full">
            {categoryChartData.length === 0 ? (
              <div className="flex items-center justify-center h-full text-slate-400 select-none text-[14px]">
                Phân tích dữ liệu sẽ xuất hiện khi bạn có đơn hàng thành công
              </div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="bg-white rounded-xl border border-slate-200/60 p-5 shadow-3xs space-y-4">
          <h4 className="font-bold text-slate-800 text-[16px]">
            Cấu trúc dòng tiền theo Phương thức thanh toán
          </h4>
        ...
        <div className="bg-white rounded-xl border border-slate-200/60 p-5 shadow-3xs space-y-4">
          <h4 className="font-bold text-slate-800 text-[16px]">
            Bảng vàng TOP 5 sản phẩm bán chạy nhất
          </h4>
          <div className="space-y-2.5">
            {bestSellingProducts.length === 0 ? (
              <div className="py-20 text-center text-slate-450 font-medium text-[14px]">
                Số liệu bán hàng sẽ tự động xếp hạng tại đây khi hoàn thành
                thanh toán.
              </div>
            ) : (
              bestSellingProducts.map((p, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between p-3 rounded-lg bg-slate-50 border border-slate-150/40 hover:border-slate-300/80 transition text-[14px]"
                >
                  <div className="flex items-center gap-3">
                    <span className="w-5 h-5 bg-blue-600 text-white rounded-full flex items-center justify-center font-extrabold text-[11px] shrink-0 shadow-xs">
                      {idx + 1}
                    </span>
                    <div>
      ...
      <div
        className="bg-white rounded-xl border border-slate-200/60 p-6 shadow-3xs space-y-6 animate-fade-in"
        id="stock-forecasting-card"
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-extrabold text-slate-800 text-[18px]">
                📊 Dự kiến cạn kiệt hàng hoá & Tốc độ tiêu thụ (Forecasting)
              </h4>
              <p className="text-[12px] text-slate-450 font-semibold mt-0.5">
                Dựa trên tốc độ tiêu thụ thực tế 3 tháng qua để tính toán ngày
                dự kiến đứt gãy và dự phóng lượng hàng tồn kho.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 max-w-sm w-full">
            <label
              htmlFor="forecast-variant-select"
              className="text-[14px] font-bold text-slate-500 shrink-0"
            >
              Mẫu hàng:
            </label>
            <select
              ...
              className="flex-1 bg-slate-50 border border-slate-200 text-slate-700 rounded-lg px-2.5 py-1.5 text-[14px] font-semibold focus:outline-none focus:border-indigo-500 cursor-pointer"
            >
        ...
        {selectedItem ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-150/40 relative overflow-hidden">
                ...
                <div className="flex items-baseline gap-1.5 mt-1">
                  <h3 className="text-2xl font-black text-slate-850">
                    {selectedItem.variant.available}
                  </h3>
                  <span className="text-[14px] text-slate-400 font-semibold">
                    / {selectedItem.variant.stock} tổng
                  </span>
                </div>
                <div
                  className={`text-[11px] font-bold mt-2 ${selectedItem.variant.available <= (config.lowStockThreshold ?? 5) ? "text-rose-500" : "text-slate-450"}`}
                >
              ...
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-150/40">
                <span className="text-[11px] font-bold text-slate-400 block uppercase">
                  LƯỢNG BÁN 90 NGÀY QUA
                </span>
                <h3 className="text-2xl font-black text-indigo-600 mt-1">
                  {forecastData.soldQty90d} sản phẩm
                </h3>
                <div className="text-[10.5px] text-slate-500 font-medium mt-1.5">
                  Tốc độ bán bình quân:{" "}
                  <b className="text-indigo-700">
                    {forecastData.dailyVelocity.toFixed(3)}
                  </b>{" "}
                  cái / ngày
                </div>
              </div>
              ...
              >
                <span className="text-[11px] font-bold opacity-60 block uppercase">
                  DỰ BÁO CẠN KIỆT KHO
                </span>
                <h3 className="text-xl font-black mt-1 leading-tight truncate">
                  {forecastData.depletionDateStr}
                </h3>

                <div className="text-[12px] font-bold mt-2 flex items-center gap-1.5">
                  <Clock size={13} />
                  <span>
            ...
            <div className="lg:col-span-2 space-y-2.5">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between text-[14px] px-2 gap-2">
                <span className="font-bold text-slate-700">
                  Đồ thị diễn biến & Xu hướng trữ lượng tồn kho
                </span>
                <div className="flex flex-wrap items-center gap-3.5 text-[11px] font-semibold text-slate-500">
                  <span className="flex items-center gap-1">
                    <span className="w-2.5 h-1 bg-blue-500 rounded-sm inline-block"></span>{" "}
                    Lịch sử lưu kho
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-2.5 h-1 border-t-2 border-dashed border-orange-500 inline-block"></span>{" "}
                    Dự báo tương lai
                  </span>
                </div>
              </div>
                    ...
                    <Tooltip
                      content={({ active, payload }) => {
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div className="bg-white border border-slate-200/80 p-2.5 rounded-lg shadow-xl text-[12px] text-slate-700 leading-normal font-sans">
                              <p className="font-bold text-slate-800 border-b border-slate-100 pb-1 mb-1">
                                Ngày: {item.date}
                              </p>
                              {actualVal !== null && (
                                <p className="text-blue-600 font-semibold">
                                  Tồn thực tế: {actualVal} cái
                                </p>
                              )}
                              {forecastVal !== null && (
                                <p className="text-orange-500 font-semibold">
                                  Dự báo trữ lượng: {Math.round(forecastVal)}{" "}
                                  cái
                                </p>
                              )}
                              <p className="text-[11px] mt-1 text-slate-400 font-medium italic">
                                {actualVal === null
                                  ? "Đường dự toán xu hướng tiêu thụ"
                                  : "Đã ghi nhận trong kho"}
                              </p>
                            </div>
                          );
                        }
              ...
              {forecastData.daysToDepletion <= 15 && (
                <div className="p-3 bg-rose-50 border border-rose-100/80 rounded-lg text-[12px] font-semibold text-rose-800 flex items-center justify-between">
                  <span>
                    ⚠️ Nguy cơ: Sản phẩm này có rủi ro cạn kiệt cao trong vòng
                    15 ngày! Đề xuất bổ sung kịp thời.
                  </span>
                  <div className="bg-rose-600 text-white font-extrabold text-[11px] px-2 py-1 rounded">
                    Yêu cầu nhập hàng
                  </div>
                </div>
              )}
          ...
          <div className="py-12 text-center text-slate-400 font-semibold text-[14px]">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 font-semibold mt-0.5">
                Tự động phát hiện các mẫu mã, SKU có số lượng khả dụng dưới 5
                sản phẩm hằng ngày
              </p>
            </div>
          </div>
          <span className="bg-rose-50 border border-rose-100 text-rose-700 text-[11px] font-black px-2.5 py-1 rounded-full shrink-0">
            {lowInventoryVariants.length} Cảnh báo
          </span>
        </div>

        {lowInventoryVariants.length === 0 ? (
          <div className="py-8 text-center text-slate-450 font-medium text-[14px]">
            🎉 Tuyệt vời! Tất cả sản phẩm hiện đều có lượng tồn kho an toàn
            (trên 5 món).
          </div>
        ) : (
          <div className="overflow-x-auto rounded-lg border border-slate-150">
            <table className="w-full text-left border-collapse min-w-[600px] text-[14px]">
              <thead>
                <tr className="border-b border-slate-150 text-[11px] font-bold text-slate-400 uppercase tracking-wider bg-slate-50">
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            ...
                            <span className="text-[11px] text-slate-400 block mt-0.5">
                              {variant.name.split(" - ")[1] || "Bản mặc định"}
                            </span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-4 text-right font-mono">
                        <span
                          className={`font-black tracking-tight ${severity === "critical" ? "text-rose-600" : severity === "high" ? "text-amber-600" : "text-slate-700"}`}
                        >
                          {variant.available}
                        </span>
                      </td>
                      <td className="py-3 px-4 text-center">
                        {severity === "critical" ? (
                          <span className="bg-rose-50 text-rose-700 border border-rose-100 text-[11px] font-black px-2 py-0.5 rounded-md uppercase tracking-wider inline-flex items-center gap-1">
                            Hết hàng 🚨
                          </span>
                        ) : severity === "high" ? (
                          <span className="bg-amber-50 text-amber-700 border border-amber-100 text-[11px] font-black px-2 py-0.5 rounded-md uppercase tracking-wider inline-flex items-center gap-1">
                            Cực thấp ⚠️
                          </span>
                        ) : (
                          <span className="bg-blue-50 text-blue-700 border border-blue-100 text-[11px] font-black px-2 py-0.5 rounded-md uppercase tracking-wider inline-flex items-center gap-1">
                            Cần Nhập 📦
                          </span>
                        )}
                      </td>
