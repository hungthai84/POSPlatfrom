  ...
  const addItem = (v: Variant) => {
    ...
    if (existing) {
      ...
      setNewCombo({
        ...
        items: [
          ...newCombo.items,
          { productId: v.productId, variantId: v.id, quantity: 1 },
        ],
      });
    }
    setProductSearch("");
  ...
  const originalPrice = useMemo(() => {
    return newCombo.items.reduce((sum, item) => {
      const variant = allVariants.find(
        (av) => av.variant.id === item.variantId,
      )?.variant;
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="space-y-0.5">
          ...
          <p className="text-[14px] text-slate-500">
            Tạo các gói sản phẩm khuyến mãi để tăng giá trị đơn hàng.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filteredCombos.length === 0 ? (
          ...
          filteredCombos.map((cb) => (
            <div
              key={cb.id}
              className="bg-white rounded-2xl border border-slate-200/60 shadow-3xs overflow-hidden flex flex-col group hover:shadow-md transition"
            >
              <div className="p-5 flex justify-between items-start gap-4 border-b border-slate-50">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span
                      className={`w-2 h-2 rounded-full ${cb.isActive ? "bg-emerald-500" : "bg-slate-300"}`}
                    />
                ...
                <div className="text-right">
                  <p className="text-[14px] font-black text-blue-600">
                    {formatCurrency(cb.price)}
                  </p>
                  <p className="text-[11px] text-slate-400 line-through">
                    {formatCurrency(cb.originalPrice)}
                  </p>
                </div>
              </div>
              ...
              <div className="p-4 flex-1 space-y-3">
                  ...
                  <div className="space-y-1">
                    {cb.items.map((item, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className="flex items-center justify-between p-2 rounded-lg bg-slate-50 text-[12px] font-medium text-slate-600 border border-slate-100"
                        >
                          <div className="flex items-center gap-2">
                            <Layers size={12} className="text-slate-400" />
                            <span className="truncate max-w-[200px]">
                              {variant?.name || "Sản phẩm lỗi"}
                            </span>
                          </div>
                ...
                {cb.description && (
                  <div className="bg-blue-50/50 p-2.5 rounded-lg border border-blue-100/50">
                    <p className="text-[12px] text-blue-700/80 italic leading-relaxed">
                      "{cb.description}"
                    </p>
                  </div>
                )}
              ...
              <div className="px-5 py-3 bg-slate-50/50 border-t border-slate-100 flex justify-between items-center">
                ...
                <button className="flex items-center gap-1.5 text-blue-600 text-[11px] font-black hover:underline">
                  <span>Chi tiết</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl border border-slate-200 overflow-hidden slide-in-from-bottom-4 animate-in duration-300 flex flex-col max-h-[90vh]">
              ...
              <div className="flex-1 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      placeholder="CB-XXXX"
                      className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-[16px] focus:outline-none focus:border-blue-500 font-mono font-bold transition"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-[16px] font-black text-blue-600 focus:outline-none focus:border-blue-500"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-[16px] focus:outline-none focus:border-blue-500 font-medium transition resize-none"
                  />
                </div>
                ...
                <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-100 flex items-start gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-4 text-[11px] text-emerald-600 font-medium">
                      <span>
                        Tổng tiền hàng: <b>{formatCurrency(originalPrice)}</b>
                      </span>
                      <span>
                        Giảm:{" "}
                        <b className="text-emerald-700">
                          {Math.max(
                            0,
                            100 - (newCombo.price / originalPrice) * 100,
                          ).toFixed(0)}
                          %
                        </b>
                      </span>
              ...
              <div className="flex-1 space-y-4">
                <div className="space-y-1.5 relative">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-10 pr-4 py-2.5 text-[16px] focus:outline-none focus:border-blue-500 font-medium transition"
                    />
                    <Search
                      size={16}
                      ...
                      {filteredVariants.map((item, idx) => (
                        ...
                        >
                          ...
                          <div className="text-right shrink-0">
                            ...
                            <p
                              className={`text-[11px] font-bold ${item.variant.available > 0 ? "text-emerald-500" : "text-red-400"}`}
                            >
                ...
                <div className="space-y-2">
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider px-1">
                    Danh sách chọn ({newCombo.items.length})
                  </label>
                    ...
                    {newCombo.items.length === 0 ? (
                      ...
                      newCombo.items.map((item, idx) => {
                        ...
                        return (
                          <div
                            key={idx}
                            className="bg-white p-2.5 rounded-xl border border-slate-200 shadow-3xs flex items-center justify-between gap-3 group animate-in zoom-in-95"
                          >
                            ...
                            <div className="flex items-center gap-2">
                              <div className="flex items-center bg-slate-100 rounded-lg px-1 py-0.5">
                                ...
                                <span className="w-8 text-center text-[14px] font-black text-slate-800">
                                  {item.quantity}
                                </span>
                                <button
                                  onClick={() =>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-100 flex gap-3">
              <button
                onClick={() => setIsModalOpen(false)}
                className="flex-1 px-4 py-2 bg-white border border-slate-200 text-slate-600 font-bold text-[14px] rounded-xl hover:bg-slate-100 transition shadow-xs"
              >
                Hủy bỏ
              </button>
              <button
                ...
                disabled={
                  !newCombo.name || !newCombo.sku || newCombo.items.length === 0
                }
                className="flex-1 px-4 py-2 bg-blue-600 text-white font-bold text-[14px] rounded-xl hover:bg-blue-700 transition shadow-lg shadow-blue-200 disabled:opacity-50 disabled:shadow-none"
              >
                Hoàn tất & Kích hoạt
              </button>
            </div>
