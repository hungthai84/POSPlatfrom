  ...
  return (
    <div className="p-6 space-y-6 bg-slate-50 min-h-[calc(100vh-4rem)]">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-extrabold text-slate-800 tracking-tight tracking-tighter">
            Công Nợ & Phải Trả
          </h2>
          <p className="text-[14px] text-slate-500">
            Quản lý nợ khách hàng và nợ nhà cung cấp.
          </p>
        </div>
        <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-3">
          ...
          <div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">
              Tổng Nợ {activeTab === "customer" ? "KHÁCH" : "NCC"}
            </p>
      ...
      <div className="flex gap-1 bg-slate-200/50 p-1 rounded-2xl w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-6 py-2 rounded-xl text-[14px] font-black transition-all ${activeTab === "customer" ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
        >
          <UserCheck size={14} />
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-2 rounded-xl text-[14px] font-black transition-all ${activeTab === "supplier" ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
        >
          <Building2 size={14} />
          ...
          >
            ...
            <div className="px-5 py-4 border-y border-slate-50 flex justify-between items-center bg-slate-50/30">
              <div>
                ...
                <p
                  className={`text-[20px] font-black ${debt.status === "urgent" ? "text-red-500" : "text-slate-800"}`}
                >
            ...
            <div className="p-4 flex items-center justify-between text-[11px] font-bold text-slate-500">
              <div className="flex items-center gap-1.5">
                <Clock size={12} className="text-slate-400" />
                <span>
                  Cập nhật:{" "}
                  {new Date(debt.lastUpdated).toLocaleDateString("vi-VN")}
                </span>
              </div>
