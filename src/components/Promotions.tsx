...
const INITIAL_PROMOTIONS: Promotion[] = [
  {
    ...
    description: 'Giảm ngay 10% tổng đơn hàng có giá trị tối thiểu từ 100K. Giảm tối đa 50K.',
    type: 'percentage',
    value: 10,
    maxDiscount: 50000,
  ...
  },
  {
    ...
    description: 'Khấu trừ trực tiếp 100K cho đơn hàng từ 550K áp dụng riêng cho quý khách hàng VIP.',
    type: 'fixed_amount',
    value: 100000,
  ...
  },
  {
    ...
    description: 'Miễn phí tối đa 30K phí vận chuyển bưu tá cho hóa đơn mua hàng từ 300K.',
    type: 'free_shipping',
    value: 30000,
  ...
  },
  {
    ...
    description: 'Giảm giá cực sâu 15% tổng hóa đơn cho các đơn hàng bán buôn tối thiểu từ 2 Triệu.',
    type: 'percentage',
    value: 15,
    maxDiscount: 500000,
  ...
  },
  {
    ...
    description: 'Giảm trực tiếp 20.000đ cho đơn hàng đầu tiên làm quen với Pancake POS.',
    type: 'fixed_amount',
    value: 20000,
    minOrderValue: 50000,
    startDate: '2026-06-01',
...
export default function Promotions() {
  ...
  const stats = {
    ...
    activeCount: promotions.filter(p => {
      ...
      return p.isActive && start <= now && end >= now && (!p.usageLimit || p.usageCount < p.usageLimit);
    }).length,
    ...
    expiredCount: promotions.filter(p => {
      const now = new Date();
      const end = new Date(p.endDate);
      return !p.isActive || end < now || (!!p.usageLimit && p.usageCount >= p.usageLimit);
    }).length,
  ...
  const handleDeletePromo = (id: string) => {
    if (window.confirm('Bạn có chắc chắn muốn xóa chương trình khuyến mãi này không? Người mua sẽ không thể áp dụng mã này nữa.')) {
  ...
  const handleSavePromo = (e: React.FormEvent) => {
    ...
    const finalPromo: Promotion = {
      ...
      description: editingPromo.description || `Giảm ${editingPromo.value}${editingPromo.type === 'percentage' ? '%' : 'đ'} cho đơn hàng phù hợp.`,
      type: editingPromo.type || 'percentage',
      value: Number(editingPromo.value) || 0,
      minOrderValue: Number(editingPromo.minOrderValue) || 0,
      maxDiscount: editingPromo.maxDiscount ? Number(editingPromo.maxDiscount) : undefined,
      ...
      usageLimit: editingPromo.usageLimit ? Number(editingPromo.usageLimit) : undefined,
    ...
    const isDuplicate = promotions.some(p => p.code === finalPromo.code && p.id !== finalPromo.id);
    if (isDuplicate) {
      alert(`Mã khuyến mãi "${finalPromo.code}" đã tồn tại! Vui lòng chọn mã khác.`);
      return;
    }
  ...
  const filteredPromotions = promotions.filter(p => {
    ...
    if (statusFilter === 'active') {
      const isReallyActive = p.isActive && start <= now && end >= now && (!p.usageLimit || p.usageCount < p.usageLimit);
      if (!isReallyActive) return false;
    } else if (statusFilter === 'scheduled') {
      const isScheduled = p.isActive && start > now;
      if (!isScheduled) return false;
    } else if (statusFilter === 'expired') {
      const isExpired = !p.isActive || end < now || (!!p.usageLimit && p.usageCount >= p.usageLimit);
  ...
  const getTypeName = (type: string) => {
    switch (type) {
      case 'percentage': return 'Khấu trừ %';
      case 'fixed_amount': return 'Giảm số tiền thẳng';
      case 'free_shipping': return 'Freeship bưu tá';
  ...
  const getTargetName = (target?: string) => {
    switch (target) {
      case 'all': return 'Toàn bộ khách';
      case 'regular': return 'Phổ thông';
      case 'vip': return 'Vực VIP';
      case 'wholesale': return 'Khách mua sỉ';
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 text-slate-700 font-sans" id="promotions-module-root">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-5">
        <div>
          ...
          <p className="text-slate-500 text-xs mt-1">
            Thiết lập mã voucher chiết khấu đơn hàng, giảm giá % hóa đơn hoặc vận chuyển miễn phí đồng bộ với cổng Pancake POS.
          </p>
        </div>
        ...
        <div>
          <button
            ...
            className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold rounded-lg text-xs transition duration-150 flex items-center gap-1.5 shadow-md shadow-indigo-500/10 cursor-pointer"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200/60 p-5 shadow-3xs flex items-start justify-between relative overflow-hidden group">
          <div className="space-y-1">
            <span className="text-[10px] uppercase font-extrabold text-slate-400 tracking-wider">Tổng số chương trình</span>
            <div className="text-xl font-black text-slate-850 tracking-tight">
              {stats.totalCampaigns}
              <span className="text-xs font-semibold ml-1.5 text-slate-500">campaigns</span>
            </div>
            <p className="text-[10px] text-slate-450 font-bold">Lưu trong cơ sở dữ liệu bán hàng</p>
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/60 p-5 shadow-3xs flex items-start justify-between relative overflow-hidden group">
          <div className="space-y-1">
            <span className="text-[10px] uppercase font-extrabold text-slate-400 tracking-wider">Chiến dịch đang chạy</span>
            <div className="text-xl font-black text-emerald-600 tracking-tight">
              {stats.activeCount}
              <span className="text-xs font-semibold ml-1.5 text-emerald-500">đang bật</span>
            </div>
            <p className="text-[10px] text-emerald-600 font-extrabold">✓ Khách hàng có thể quét áp mã</p>
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/60 p-5 shadow-3xs flex items-start justify-between relative overflow-hidden group">
          <div className="space-y-1">
            <span className="text-[10px] uppercase font-extrabold text-slate-400 tracking-wider">Lượt áp mã thành công</span>
            <div className="text-xl font-black text-indigo-600 tracking-tight">
              {stats.totalUsages.toLocaleString()}
              <span className="text-xs font-semibold ml-1 text-slate-450">lượt</span>
            </div>
            <p className="text-[10px] text-slate-450 font-semibold">Tự động cộng dồn doanh thu</p>
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/60 p-5 shadow-3xs flex items-start justify-between relative overflow-hidden group/item">
          <div className="space-y-1 w-full">
            <span className="text-[10px] uppercase font-extrabold text-slate-400 tracking-wider">Cơ cấu thể loại giảm</span>
            <div className="grid grid-cols-2 gap-1.5 pt-1.5 text-[9.5px] text-slate-500 font-bold">
              <div className="flex items-center gap-1">
                <span className="w-1.5 h-1.5 rounded-full bg-blue-500"></span>
                <span>% Chiết khấu: <b className="text-slate-800">{stats.typeGroups.percentage}</b></span>
              </div>
              <div className="flex items-center gap-1">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                <span>Khấu trừ trực tiếp: <b className="text-slate-800">{stats.typeGroups.fixed_amount}</b></span>
              </div>
              <div className="flex items-center gap-1">
                <span className="w-1.5 h-1.5 rounded-full bg-amber-500"></span>
                <span>Freeship: <b className="text-slate-800">{stats.typeGroups.free_shipping}</b></span>
              </div>
              <div className="flex items-center gap-1">
                <span className="w-1.5 h-1.5 rounded-full bg-pink-500"></span>
                <span>Quà tặng: <b className="text-slate-800">{stats.typeGroups.buy_x_get_y}</b></span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/60 p-4 shadow-3xs">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-1 p-1 bg-slate-100 rounded-xl w-full lg:w-auto">
            ...
            >
              <span>Tất cả</span>
              <span className={`text-[10px] px-1.5 py-0.5 rounded-md ${statusFilter === 'all' ? 'bg-slate-100 text-slate-700' : 'bg-slate-200/70 text-slate-600'}`}>
                {stats.totalCampaigns}
              </span>
            </button>
            ...
            >
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
              <span>Đang chạy</span>
              <span className={`text-[10px] px-1.5 py-0.5 rounded-md ${statusFilter === 'active' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-200/70 text-slate-600'}`}>
                {stats.activeCount}
              </span>
            </button>
            ...
            >
              ...
              <span className={`text-[10px] px-1.5 py-0.5 rounded-md ${statusFilter === 'scheduled' ? 'bg-indigo-50 text-indigo-650 font-bold' : 'bg-slate-200/70 text-slate-600'}`}>
            ...
            >
              ...
              <span className={`text-[10px] px-1.5 py-0.5 rounded-md ${statusFilter === 'expired' ? 'bg-amber-55 text-amber-700 font-bold' : 'bg-slate-200/70 text-slate-600'}`}>
                {stats.expiredCount}
              </span>
            </button>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:flex-1 lg:justify-end">
            ...
            <div className="relative flex-1 w-full max-w-md">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 rounded-lg border border-slate-200 text-xs focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 outline-hidden font-semibold text-slate-700"
                value={searchTerm}
            ...
            <div className="w-full sm:w-auto">
              <select
                className="w-full sm:w-auto p-2 border border-slate-200 rounded-lg text-xs font-bold outline-hidden text-slate-600 bg-slate-50/50 cursor-pointer"
                value={typeFilter}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredPromotions.length === 0 ? (
          <div className="col-span-full bg-slate-50/50 rounded-2xl border border-dashed border-slate-250 py-16 text-center text-slate-400 space-y-3">
            ...
            <div className="space-y-1">
              <h4 className="font-extrabold text-slate-700 text-xs uppercase">Không tìm thấy khuyến mãi phù hợp</h4>
              <p className="text-[10px] text-slate-450 max-w-sm leading-normal mx-auto">
                Hãy thử đổi từ khóa tìm kiếm, thay đổi một bộ lọc trạng thái hoặc bấm "Tạo chiến dịch mới" để tiến hành thêm mã voucher.
              </p>
            </div>
          </div>
        ) : (
          filteredPromotions.map(p => {
            ...
            const start = new Date(p.startDate);
            const end = new Date(p.endDate);
            const isFullyUsed = p.usageLimit ? p.usageCount >= p.usageLimit : false;
            const isNotStarted = start > now;
            const isExpired = end < now;
            
            // Render specific status badge
            let statusBadge = (
              <span className="inline-flex items-center gap-1.5 text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-120">
            ...
            if (!p.isActive) {
              statusBadge = (
                <span className="inline-flex items-center gap-1.5 text-[10px] font-bold text-slate-500 bg-slate-50 px-2 py-0.5 rounded border border-slate-220">
              ...
              statusBadge = (
                <span className="inline-flex items-center gap-1.5 text-[10px] font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded border border-red-150">
              ...
              statusBadge = (
                <span className="inline-flex items-center gap-1.5 text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded border border-amber-120">
                  <span className="w-1.5 h-1.5 rounded-full bg-amber-500"></span>
              ...
              statusBadge = (
                <span className="inline-flex items-center gap-1.5 text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-150">
                  <span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span>
            ...
            return (
              <div 
                ...
                className={`bg-white rounded-xl border p-5 shadow-3xs flex flex-col justify-between transition-all duration-300 relative overflow-hidden group ${
                  p.isActive ? 'border-slate-200/60 hover:border-slate-300' : 'border-slate-200 bg-slate-50/20 opacity-80'
                }`}
              >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-start gap-2.5 pt-1.5">
                    ...
                    <div className={`p-2.5 rounded-xl border shrink-0 ${
                      p.type === 'percentage' ? 'bg-blue-50 text-blue-600 border-blue-100' :
                      p.type === 'fixed_amount' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                      p.type === 'free_shipping' ? 'bg-amber-50 text-amber-600 border-amber-100' :
                    ...
                    <div className="min-w-0">
                      <h3 className="font-extrabold text-slate-800 text-sm tracking-tight leading-tight group-hover:text-indigo-650 transition">
                        {p.name}
                      </h3>
                      {/* Bold code badge */}
                      <span className="inline-block mt-1 font-mono text-xs font-black tracking-wider text-indigo-600 bg-indigo-50/60 border border-indigo-100 px-2 py-0.5 rounded uppercase">
                        {p.code}
                      </span>
                    </div>
                  </div>
                  ...
                  <p className="text-[11px] text-slate-500 font-semibold leading-relaxed pt-1 select-all h-10 overflow-hidden line-clamp-2">
                    {p.description}
                  </p>

                  {/* Conditions & limits brief bullet metadata labels */}
                  <div className="bg-slate-50/60 rounded-lg border border-slate-100 p-2.5 space-y-1.5 text-[10px] text-slate-500 font-bold">
                    <div className="flex items-center justify-between">
                      <span className="text-slate-400 font-semibold">Hình thức:</span>
                      <span className="text-slate-700 font-extrabold">{getTypeName(p.type)}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-slate-400 font-semibold">Trị giá chiết khấu:</span>
                    ...
                    {p.type === 'percentage' && p.maxDiscount && (
                      <div className="flex items-center justify-between">
                        <span className="text-slate-400 font-semibold">Giảm tối đa:</span>
                        <span className="text-slate-700 font-extrabold">{p.maxDiscount.toLocaleString()}đ</span>
                      </div>
                    )}
                    <div className="flex items-center justify-between">
                      <span className="text-slate-400 font-semibold">Đơn hàng tối thiểu:</span>
                      <span className="text-slate-700 font-extrabold">{p.minOrderValue.toLocaleString()}đ</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-slate-400 font-semibold">Nhóm đối tượng:</span>
                      <span className="text-blue-600 bg-blue-50 px-1 rounded font-extrabold text-[9px]">
                  ...
                  {p.usageLimit ? (
                    <div className="space-y-1 pt-1">
                      <div className="flex items-center justify-between text-[9px] text-slate-400 font-bold">
                        ...
                        <span className="text-slate-600">{p.usageCount} / {p.usageLimit} lượt</span>
                      </div>
                      <div className="relative w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                        <div 
                          className={`absolute top-0 left-0 h-full rounded-full ${
                            getPromoProgressPercentage(p) > 85 ? 'bg-amber-500' : 'bg-indigo-500'
                          }`}
                    ...
                    <div className="text-[9px] text-slate-400 font-bold pt-1 flex justify-between">
                      <span>Đã sử dụng: <b className="text-slate-600">{p.usageCount} lượt</b></span>
                ...
                <div className="border-t border-slate-100/80 pt-3.5 mt-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      className={`text-[10px] px-2 py-0.5 rounded-full border transition cursor-pointer font-bold ${
                        p.isActive 
      ...
      {isDrawerOpen && editingPromo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200" id="promotion-drawer-container">
          {/* Backdrop overlay */}
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs transition-opacity duration-300" onClick={() => setIsDrawerOpen(false)}></div>
          ...
          <div className="relative w-full max-w-xl bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-5 border-b border-slate-150 bg-slate-50 flex items-center justify-between">
              <div>
                <h3 className="font-extrabold text-slate-800 text-sm uppercase">
                  {editingPromo.id && promotions.some(p => p.id === editingPromo.id) 
                    ? 'Chỉnh Sửa Khuyến Mãi' 
                    : 'Thêm Khuyến Mãi Mới'}
                </h3>
                <p className="text-[10px] text-indigo-600 font-bold mt-0.5 font-mono">{editingPromo.id}</p>
              </div>
              <button
            ...
            {/* Form body */}
            <form onSubmit={handleSavePromo} className="flex-1 overflow-y-auto p-6 space-y-4 text-xs">
              {/* Promo Code section */}
              <div className="space-y-1.5">
                <div className="flex justify-between items-center">
                  <label className="font-black uppercase text-slate-500 text-[10px]">Mã khuyến mãi (Voucher Code) *</label>
                  <button
                    type="button"
                    onClick={generateRandomCode}
                    className="text-[10px] text-indigo-600 hover:underline font-extrabold inline-flex items-center gap-1 cursor-pointer"
                  >
                ...
                <input
                  ...
                  id="input-promo-code"
                />
                <p className="text-[9.5px] text-slate-450 font-bold">Mã viết liền không dấu, không khoảng cách, sẽ tự động viết hoa toàn bộ.</p>
              </div>

              {/* Campaign Title / Name */}
              <div className="space-y-1.5">
                <label className="font-black uppercase text-slate-500 text-[10px]">Tên chương trình / Chiến dịch *</label>
                <input
                  type="text"
                  required
              ...
              <div className="space-y-1.5">
                <label className="font-black uppercase text-slate-500 text-[10px]">Mô tả điều kiện &amp; Quy chuẩn áp dụng</label>
                <textarea
                  rows={2}
              ...
              <div className="grid grid-cols-2 gap-3.5 pt-1.5 border-t border-slate-100">
                <div className="space-y-1.5">
                  <label className="font-black uppercase text-slate-500 text-[10px]">Hình thức giảm</label>
                  <select
                    ...
                    onChange={(e) => setEditingPromo(prev => ({ ...prev, type: e.target.value as any, value: e.target.value === 'percentage' ? 10 : 20000 }))}
                  >
                    <option value="percentage">Chiết khấu phần trăm (%)</option>
                    <option value="fixed_amount">Trừ tiền mặt thẳng (đ)</option>
                    <option value="free_shipping">Giảm phí ship bưu tá (đ)</option>
                    <option value="buy_x_get_y">Chương trình Mua X tặng Y</option>
                  </select>
                </div>

                <div className="space-y-1.5">
                  <label className="font-black uppercase text-slate-500 text-[10px]">
                    {editingPromo.type === 'percentage' ? 'Giá trị chiết khấu (%)' :
                     editingPromo.type === 'buy_x_get_y' ? 'Số sản phẩm tặng' : 'Mức giảm (VNĐ)'} *
                  </label>
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => setEditingPromo(prev => ({ ...prev, value: Number(e.target.value) }))}
                    ...
                    <span className="absolute right-3 top-1/2 transform -translate-y-1/2 font-black text-slate-400">
                      {editingPromo.type === 'percentage' ? '%' :
                       editingPromo.type === 'buy_x_get_y' ? 'SP' : 'đ'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              {editingPromo.type === 'percentage' && (
                <div className="space-y-1.5">
                  <label className="font-black uppercase text-slate-500 text-[10px]">Giới hạn số tiền giảm tối đa (VNĐ)</label>
                  <div className="relative">
                    <input
                      type="number"
                      ...
                      onChange={(e) => setEditingPromo(prev => ({ ...prev, maxDiscount: e.target.value ? Number(e.target.value) : undefined }))}
                      id="input-promo-max-discount"
                    />
                    <span className="absolute right-3.5 top-1/2 transform -translate-y-1/2 font-black text-slate-400">đ</span>
                  </div>
                </div>
              )}
              ...
              {/* Minimum Order Value */}
              <div className="space-y-1.5">
                <label className="font-black uppercase text-slate-500 text-[10px]">Giá trị đơn hàng tối thiểu cần thu (VNĐ)</label>
                <div className="relative">
                  <input
                    type="number"
                    ...
                    onChange={(e) => setEditingPromo(prev => ({ ...prev, minOrderValue: Number(e.target.value) }))}
              ...
              <div className="space-y-1.5">
                <label className="font-black uppercase text-slate-500 text-[10px]">Nhóm đối tượng khách hàng áp dụng</label>
                <select
                  className="w-full border border-slate-200 rounded-lg p-2.5 font-extrabold outline-hidden text-slate-700 bg-white"
                  value={editingPromo.targetCustomerType || 'all'}
                  onChange={(e: any) => setEditingPromo(prev => ({ ...prev, targetCustomerType: e.target.value }))}
                >
                  <option value="all">Mọi nhóm đối tượng khách hàng (Tất cả)</option>
                  <option value="regular">Chỉ thuộc phân khúc Phổ thông</option>
                  <option value="vip">Chỉ dành đặc quyền cho Thành viên VIP</option>
                  <option value="wholesale">Chỉ hiển thị cho Đối tác mua sỉ</option>
                </select>
              </div>

              {/* Total Usage Limit */}
              <div className="space-y-1.5">
                <label className="font-black uppercase text-slate-500 text-[10px]">Tổng lượt phát hành tối đa (Usage Limit)</label>
                <input
                  type="number"
                  ...
                  onChange={(e) => setEditingPromo(prev => ({ ...prev, usageLimit: e.target.value ? Number(e.target.value) : undefined }))}
              ...
              <div className="grid grid-cols-2 gap-3.5 pt-1.5 border-t border-slate-100">
                <div className="space-y-1.5">
                  <label className="font-black uppercase text-slate-500 text-[10px]">Ngày bắt đầu hiệu lực</label>
                  <input
                    type="date"
                    required
                ...
                <div className="space-y-1.5">
                  <label className="font-black uppercase text-slate-500 text-[10px]">Ngày hết hạn chiến dịch</label>
                  <input
                    type="date"
                    required
                    className="w-full border border-slate-200 rounded-lg p-2 font-bold outline-hidden text-slate-700"
              ...
              <div className="flex items-center justify-between p-3.5 bg-slate-50 rounded-xl border border-slate-150">
                <div className="space-y-0.5">
                  <label className="font-extrabold text-slate-800 text-[11px]">Trạng thái chiến dịch</label>
                  <p className="text-[9px] text-slate-450 leading-none">Cho phép kích hoạt áp dụng lập tức tại quầy thu ngân</p>
                </div>
                <button
                  type="button"
                  onClick={() => setEditingPromo(prev => ({ ...prev, isActive: !prev?.isActive }))}
              ...
              <div className="pt-4 border-t border-slate-100 flex gap-2.5 justify-end">
                <button
                  type="button"
                  onClick={() => setIsDrawerOpen(false)}
                  className="px-4 py-2 border border-slate-200 hover:bg-slate-50 rounded-lg text-xs font-bold transition duration-150 cursor-pointer text-slate-600"
                >
                  Đóng bỏ qua
                </button>
                <button
                  type="submit"
                  className="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-extrabold rounded-lg transition duration-150 shadow-md shadow-indigo-500/10 cursor-pointer"
