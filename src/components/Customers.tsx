      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="space-y-0.5">
          <h2 className="text-[20px] font-extrabold text-slate-800 tracking-tight">
            Hồ Sơ Khách Hàng & Thành Viên
          </h2>
          <p className="text-[14px] text-slate-450">
            Quản lý hạng mức thành viên, lịch sử chi tiêu tại quầy POS.
          </p>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl border border-slate-200/60 p-4 shadow-3xs flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          <input
            ...
            className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-10 pr-4 py-2 text-[14px] text-slate-705 font-medium focus:outline-none focus:border-blue-500 focus:bg-white transition"
          />
          <Search
            size={14}
        ...
        <div className="flex bg-slate-100 p-1 rounded-lg border border-slate-150 space-x-1 shrink-0">
          {[
            ...
            <button
              ...
              className={`text-[11px] font-bold px-2.5 py-1.5 rounded transition ${
                selectedType === st.id
              ...
              <tbody>
                {filteredCustomers.length === 0 ? (
                  <tr>
                    <td
                      colSpan={7}
                      className="py-12 text-center text-slate-400 font-semibold select-none"
                    >
                        ...
                        >
                          <button
                            onClick={() => {
                              if (
                                confirm(
                                  `Xóa khách hàng ${cust.name} khỏi cơ sở dữ liệu phân tích?`,
                                )
                              ) {
          ...
          {activeCustomer ? (
            <div className="bg-white rounded-xl border border-slate-200/60 p-5 shadow-3xs space-y-5 sticky top-24 animate-fade-in-right font-medium">
              <div className="text-center pb-4 border-b border-slate-100 flex flex-col items-center space-y-2">
                <div
                  className={`w-12 h-12 rounded-full flex items-center justify-center text-white text-[20px] font-black ${
                ...
                <div>
                  ...
                  <span className="text-[11px] font-semibold text-slate-400 uppercase font-mono mt-0.5 block">
              ...
              <div className="space-y-3 text-[14px] text-slate-650">
              ...
              <div className="pt-4 border-t border-slate-100 grid grid-cols-2 gap-3 text-center">
                <div className="p-3 bg-slate-50/75 border border-slate-100 rounded-lg text-[14px]">
                  <span className="text-[11px] text-slate-400 uppercase tracking-wider block font-bold">
                    Số lần mua tại POS
                  </span>
                  <span className="text-[18px] font-black text-slate-900 mt-1 block">
                    {activeCustomer.orderCount} lần
                  </span>
                </div>
                <div className="p-3 bg-slate-50/75 border border-slate-100 rounded-lg text-[14px]">
                  <span className="text-[11px] text-slate-400 uppercase tracking-wider block font-bold">
                  ...
                  <span className="text-[18px] font-black text-emerald-800 mt-1 block">
              ...
              <div className="p-3 bg-blue-50/40 text-[12px] text-blue-800 rounded border border-blue-100/40 space-y-1">
                <div className="flex items-center gap-1 font-bold">
                  <History size={13} className="text-blue-600 shrink-0" />
                  <span>Mối mua sắm định đám</span>
                </div>
                <p className="text-blue-700 leading-relaxed text-[12px]">
                  Khách hàng ưa thích sử dụng phương pháp{" "}
                  <strong>Chuyển khoản</strong> thanh toán và thường ghé mua s
                  sắm vào ngày cuối tuần.
                </p>
              </div>

              {/* Past orders list */}
              <div className="pt-2">
                <h4 className="font-bold text-[16px] text-slate-800 mb-3 flex items-center gap-2">
                  <ShoppingBag size={14} /> Lịch sử mua hàng
                </h4>
                <div className="space-y-2 max-h-60 overflow-y-auto pr-1 scrollbar-thin">
                  {orders.filter((o) => o.customer?.id === activeCustomer.id)
                    .length === 0 ? (
                    ...
                    orders
                      .filter((o) => o.customer?.id === activeCustomer.id)
                      .map((o) => (
                        <div
                          key={o.id}
                          className="bg-slate-50 border border-slate-100 rounded-lg p-3 text-[14px] flex justify-between items-center hover:shadow-xs transition"
                        >
                          <div>
            ...
            <div className="bg-white rounded-xl border border-slate-200/60 p-10 text-center text-slate-400 font-semibold text-[14px] h-64 flex flex-col items-center justify-center sticky top-24 select-none">
              <Users2 size={28} className="text-slate-350 stroke-1 mb-2" />
              <span>
                Hãy chọn 1 người trong danh mục thành viên để xem hồ sơ giao
                dịch chi tiết.
              </span>
            </div>
          )}
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4 backdrop-blur-xs select-none">
          <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-2xl border border-slate-150 animate-zoom-in">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3 mb-4">
              <h3 className="font-extrabold text-slate-800 text-[16px]">
                Thêm mới hồ sơ khách hàng thành viên
              </h3>
              <button
            ...
            <form
              ...
              className="space-y-4 text-[14px] font-bold"
            >
