...
const BANNER_PRESETS = [
  {
    id: "minimal-light",
    name: "Sáng tối giản",
    class: "bg-slate-50 text-slate-800 border-b border-light",
  },
  {
    id: "indigo-gradient",
    name: "Gradient Lam Tím",
    class:
      "bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600 text-white",
  },
  {
    id: "emerald-fresh",
    name: "Xanh Lá Tươi Mới",
    class: "bg-gradient-to-r from-emerald-600 to-teal-500 text-white",
  },
  {
    id: "rose-coral",
    name: "Hồng San Hô",
    class:
      "bg-gradient-to-r from-rose-500 to-amber-500 text-white animate-pulse-slow",
  },
];
...
export default function WebsiteBuilder({
  ...
  const [webConfig, setWebConfig] = useState<WebConfig>(() => {
    ...
    return {
      ...
      subtitle:
        "Khám phá bộ sưu tập sản phẩm mới nhất với mức giá ưu đãi cực kì hấp dẫn.",
  ...
  >([]);
  const [selectedWebProduct, setSelectedWebProduct] = useState<Product | null>(
    null,
  );
  ...
  const handleCopyLink = () => {
    ...
    navigator.clipboard.writeText(
      `https://${config.name.toLowerCase().replace(/\s+/g, "")}.larkshop.store`,
    );
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-[14px] text-slate-450 mt-1">
            Thiết kế, đồng bộ danh mục và tạo trang bán trực tuyến chuyên nghiệp
            cho cửa hàng của bạn chỉ với 1 click.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 bg-white hover:bg-slate-50 border border-slate-200 rounded-lg text-[14px] font-semibold text-slate-700 transition shadow-2xs`}
          >
            {copiedLink ? (
          ...
          <button
            onClick={() =>
              setPublishStatus((prev) =>
                prev === "draft" ? "published" : "draft",
              )
            }
            className={`flex items-center gap-1.5 px-3 py-2 ${publishStatus === "published" ? "bg-emerald-600 hover:bg-emerald-700 text-white border-emerald-700" : "bg-slate-200 text-slate-700"} rounded-lg text-[14px] font-semibold transition`}
          >
            <Sparkles size={13} />
            <span>
        ...
        <div className="lg:col-span-5 bg-white border border-slate-200/60 rounded-xl shadow-sm overflow-hidden flex flex-col max-h-[85vh]">
          <div className="p-4 border-b border-rose-50/50 bg-slate-50 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[14px] font-bold text-slate-700 uppercase tracking-wider">
                Cấu hình Web Trực Tuyến
              </span>
            </div>
            <span className="text-[11px] bg-blue-50 text-blue-600 font-semibold px-2 py-0.5 rounded-full border border-blue-100 animate-pulse-slow">
              Auto Sync Realtime
            </span>
          </div>
          ...
          <div className="p-5 overflow-y-auto space-y-5 flex-1 scrollbar-thin">
            ...
            <div className="space-y-3">
              <span className="text-[14px] font-semibold text-slate-400 block uppercase tracking-wider">
                Giao Diện Tiêu Đề
              </span>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setWebConfig((prev) => ({ ...prev, title: e.target.value }))
                  }
                  className="w-full text-[14px] border border-slate-200 rounded-lg px-3 py-2 focus:ring-1 focus:ring-blue-500 outline-none transition"
              ...
              <div>
                <label className="block text-[12px] font-bold text-slate-500 mb-1">
                  Lời chào / Khẩu hiệu (Hero Subtitle)
                </label>
                <textarea
              ...
              <div>
                <label className="block text-[12px] font-bold text-slate-500 mb-1">
                  Về chúng tôi (About section footer)
                </label>
                <textarea
            ...
            {/* Design & Color Themes */}
            <div className="space-y-3">
              <span className="text-[14px] font-semibold text-slate-400 block uppercase tracking-wider flex items-center gap-1.5">
                <Palette size={13} />
                ...
                <div className="grid grid-cols-2 gap-2">
                  {BANNER_PRESETS.map((p) => (
                    <button
                      ...
                      className={`p-2.5 rounded-lg text-left text-[14px] font-semibold border transition overflow-hidden truncate ${
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-slate-505 font-medium">
                  Sản phẩm kích hoạt: {webConfig.featuredProductIds.length}
                </span>
              </div>
              <p className="text-[11px] text-slate-400">
                Chọn những sản phẩm sẽ hiển thị công khai trên website bán hàng
                của bạn.
              </p>
              ...
              <div className="border border-slate-150 rounded-lg overflow-hidden divide-y divide-slate-100 max-h-48 overflow-y-auto bg-slate-50/50">
                {products.length === 0 ? (
                  <div className="p-4 text-center text-[14px] text-slate-400">
                    Không tìm thấy sản phẩm trong danh mục
                  </div>
                ) : (
                  products.map((p) => {
                    const isChecked = webConfig.featuredProductIds.includes(
                      p.id,
                    );
                    return (
                      <div
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <span className="text-[11px] text-slate-450 block truncate">
              ...
              <div className="grid grid-cols-1 gap-2.5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full text-[14px] border border-slate-200 rounded-lg px-2.5 py-1.5 focus:ring-1 focus:ring-blue-500 outline-none transition"
                  />
                </div>

                <div>
                  <label className="block text-[12px] font-bold text-slate-500 mb-1">
                    Tài khoản Tiktok URL
                  </label>
                  <input
                    type="text"
            ...
            <div className="border border-indigo-100 bg-indigo-50/40 rounded-xl p-3 flex gap-2.5 mt-2">
              ...
              <div className="space-y-1">
                <span className="text-[12px] font-bold text-indigo-700 block">
                  Sự phối hợp hoàn hảo!
                </span>
                <p className="text-[11px] leading-relaxed text-indigo-900">
                  Mỗi khi một người dùng gửi thông tin đặt hàng trên trang web
                  này, một đơn hàng với tag <b>WEB-xxxx</b> sẽ tự động tạo bên
                  trong POS, trừ kho và tính điểm thưởng loyalty một cách thực
                  sự!
                </p>
              </div>
            </div>
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="flex items-center justify-between px-1">
            <span className="text-[14px] font-bold text-slate-400 uppercase tracking-widest block">
              Màn hình mô phỏng thời gian thực
            </span>
          ...
          <div className="flex justify-center w-full">
            ...
            >
              ...
              <div className="bg-slate-850 px-4 py-2 border-b border-slate-800 flex items-center gap-3">
                ...
                {/* Simulated URL bar */}
                <div className="flex-1 bg-slate-800 text-[11px] text-slate-300 px-3 py-1 rounded-md border border-slate-700/50 flex items-center justify-between font-mono">
                  <span className="truncate">
                    https://{config.name.toLowerCase().replace(/\s+/g, "")}
                    .larkshop.store
                  </span>
              ...
              <div className="bg-white text-slate-800 h-[calc(100%-2.5rem)] overflow-y-auto text-left relative flex flex-col scrollbar-thin">
                ...
                <header className="sticky top-0 bg-white/95 backdrop-blur-md shadow-xs border-b border-slate-100 px-4 py-3 flex items-center justify-between z-10 shrink-0">
                  <span className="text-[16px] font-black text-slate-900 tracking-tight flex items-center gap-1.5">
                    <span
                      className={`w-2.5 h-2.5 rounded-full ${themeScheme.bg}`}
                    />
                    {config.name}
                  </span>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      ...
                      {cartTotalQty > 0 && (
                        <span
                          className={`absolute -top-1 -right-1.5 ${themeScheme.bg} text-white font-bold text-[9px] w-4 h-4 rounded-full flex items-center justify-center ring-2 ring-white animate-bounce`}
                        >
                          {cartTotalQty}
                        </span>
                      )}
                ...
                {publishStatus === "draft" ? (
                  <div className="flex-1 flex flex-col items-center justify-center p-8 bg-slate-50 text-center space-y-3">
                    ...
                    <div>
                      ...
                      <p className="text-[14px] text-slate-450 mt-1">
                        Cửa hàng hiện đang bảo trì hệ thống. Vui lòng quay lại
                        sau ít phút.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 flex flex-col">
                    ...
                    {isOrderedSuccess && (
                      <div className="bg-emerald-50 border-b border-emerald-100 p-4 shrink-0 text-center space-y-1 animate-fade-in">
                        <span className="text-[12px] font-bold text-emerald-800 block">
                          ✨ Đặt hàng thành công hoàn tất! ✨
                        </span>
                        <p className="text-[11px] text-emerald-700">
                          Mã hóa đơn trực tuyến <b>{lastGeneratedOrderId}</b> đã
                          được tạo tự động trong màn hình quản trị Đơn hàng.
                        </p>
                        <button
                          onClick={() => setIsOrderedSuccess(false)}
                          className="text-[10px] text-emerald-600 font-extrabold underline block mx-auto mt-1"
                        >
                          Đóng thông báo
                        </button>
                      </div>
                    ...
                    {/* Banner Section */}
                    <div
                      className={`p-6 text-center shrink-0 border-b border-slate-100 ${
                        BANNER_PRESETS.find(
                          (p) => p.id === webConfig.bannerPreset,
                        )?.class || BANNER_PRESETS[1].class
                      }`}
                    >
                      <span className="text-[11px] font-black tracking-widest uppercase bg-white/20 px-2 py-0.5 rounded-full inline-block backdrop-blur-xs mb-2">
                        CHÀO MỪNG QUÝ KHÁCH
                      </span>
                      <h2 className="text-[18px] sm:text-[20px] font-black tracking-tight">
                        {webConfig.title}
                      </h2>
                      <p className="text-[12px] mt-1 opacity-90 max-w-sm mx-auto leading-relaxed">
                        {webConfig.subtitle}
                      </p>
                    </div>
                    ...
                    <div className="p-3 bg-slate-50 border-b border-slate-100 space-y-2.5 shrink-0">
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full text-[11px] border border-slate-200/80 rounded-lg px-2.5 py-1 bg-white outline-none focus:border-slate-300"
                        />
                      </div>
                      ...
                      <div className="flex gap-1 overflow-x-auto pb-1 scrollbar-none shrink-0">
                        {categories.map((cat) => (
                          <button
                            ...
                            className={`text-[10px] font-bold px-2.5 py-1 rounded-full transition shrink-0 uppercase tracking-widest ${
                              activeCategory === cat
                      ...
                      {simProducts.length === 0 ? (
                        <div className="py-12 text-center text-slate-400 space-y-2">
                          ...
                          <p className="text-[11px] opacity-75">
                            Hãy tick chọn tối thiểu một sản phẩm ở menu bên
                            trái.
                          </p>
                        </div>
                      ) : (
                        <div
                          className={`grid gap-3 ${previewDevice === "mobile" ? "grid-cols-2" : "grid-cols-3"}`}
                        >
                          {simProducts.map((p) => (
                            <div
                              key={p.id}
                            ...
                            >
                              <div className="relative aspect-square shrink-0">
                                ...
                                />
                                <span
                                  className={`absolute top-1 right-1 text-[9px] font-bold px-1.5 py-0.5 rounded ${themeScheme.bg} text-white uppercase`}
                                >
                                  HOT
                                </span>
                              </div>
                              <div className="p-2 flex-1 flex flex-col justify-between bg-slate-50/20">
                                <span className="block text-[12px] font-bold text-slate-800 line-clamp-2 min-h-[30px]">
                                  {p.name}
                                </span>
                                <div className="mt-2 flex items-center justify-between">
                                  <span
                                    className={`text-[12px] font-black ${themeScheme.text}`}
                                  >
                  ...
                  <div className="absolute inset-0 bg-black/60 z-20 flex items-end animate-fade-in h-full">
                    <div className="bg-white rounded-t-2xl w-full max-h-[85%] overflow-y-auto p-4 space-y-4 animate-fade-in-right">
                      ...
                      <div className="flex justify-between items-center pb-2 border-b border-slate-100 shrink-0">
                        <span className="text-[14px] font-bold text-slate-800 max-w-[80%] truncate">
                          Chi tiết sản phẩm
                        </span>
                        <button
                          onClick={() => setSelectedWebProduct(null)}
                          className="text-[14px] font-bold text-slate-450 hover:text-slate-800"
                        >
                          Đóng
                        </button>
                      </div>
                      ...
                      <div className="flex gap-3">
                        ...
                        <div className="min-w-0">
                          <h4 className="text-[14px] font-extrabold text-slate-800 leading-snug">
                            {selectedWebProduct.name}
                          </h4>
                          <p className="text-[11px] text-slate-450 mt-1 uppercase tracking-wider">
                            {selectedWebProduct.category}
                          </p>
                          <p className="text-[14px] font-extrabold mt-1 text-slate-900">
                            {selectedWebProduct.variants
                      ...
                      <div className="space-y-1.5">
                        <label className="block text-[12px] font-bold text-slate-500">
                          Mẫu mã sản phẩm (Màu sắc / Kích thước/ Dung tích):
                        </label>
                      ...
                      <div className="space-y-1 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                        ...
                        <p className="text-[11px] text-slate-500 leading-relaxed">
                          {selectedWebProduct.description ||
                            "Sản phẩm mới chính hãng."}
                        </p>
                      </div>
                      ...
                      <button
                        ...
                        className={`w-full text-center py-2.5 rounded-xl text-[14px] font-extrabold text-white transition ${
                          selectedVariantId
                            ? `${themeScheme.bg} ${themeScheme.hover}`
                  ...
                  <div className="absolute inset-0 bg-black/50 z-20 flex items-end h-full">
                    <div className="bg-white rounded-t-2xl w-full max-h-[90%] overflow-y-auto p-4 space-y-4 flex flex-col justify-between">
                      <div>
                          ...
                          <div className="divide-y divide-slate-100 max-h-40 overflow-y-auto pr-1">
                            {webCart.map((item) => {
                              ...
                              return (
                                ...
                                >
                                  <div className="flex-1 min-w-0">
                                    <span className="block text-[12px] font-bold text-slate-700 truncate">
                                      {p.name}
                                    </span>
                                    <span className="text-[11px] text-slate-450 block truncate">
                                      Phân loại:{" "}
                                    ...
                                    <span className="text-[10px] font-bold text-blue-600 block">
                                      {v.price.toLocaleString("vi-VN")}đ/cái
                                    </span>
                                  </div>
                                  ...
                                  <div className="flex items-center gap-1">
                                    <button
                                      onClick={() =>
                                        handleCartQtyChange(p.id, v.id, -1)
                                      }
                                      className="w-5 h-5 rounded bg-slate-100 text-slate-700 font-bold text-[14px] flex items-center justify-center hover:bg-slate-200"
                                    >
                                      -
                                    </button>
                                    <span className="text-[14px] font-bold w-6 text-center">
                                      {item.quantity}
                                    </span>
                                    <button
                                      onClick={() =>
                        ...
                        {/* Summary */}
                        {webCart.length > 0 && (
                          <div className="border-t border-slate-100 pt-2.5 pb-2.5 space-y-1 font-semibold text-[12px] text-slate-600">
                            ...
                            <div className="flex justify-between text-[12px]">
                              ...
                              <span className="text-slate-800">
                                {Math.round(
                                  cartTotalPrice * config.taxRate,
                                ).toLocaleString("vi-VN")}
                                đ
                              </span>
                            </div>
                            <div className="flex justify-between text-[14px] font-bold border-t border-slate-50 pt-1.5 text-slate-900">
                              <span>Tổng cộng thanh toán:</span>
                              <span className={themeScheme.text}>
                                {(
                                  cartTotalPrice +
                                  Math.round(cartTotalPrice * config.taxRate)
                                ).toLocaleString("vi-VN")}
                                đ
                              </span>
                            </div>
                        ...
                        {webCart.length > 0 && (
                          ...
                          >
                            <span className="text-[11px] font-black text-slate-450 uppercase block tracking-wider">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <div>
                                ...
                                <input
                                  ...
                                  className="w-full text-[11px] border border-slate-200 rounded-md px-2 py-1 outline-none focus:border-slate-300"
                                />
                              </div>
                            ...
                            <div className="space-y-1">
                              ...
                              <div className="flex gap-2">
                                <button
                                  ...
                                  className={`flex-1 text-[11px] p-2 border rounded-lg text-center font-bold transition ${
                            ...
                            <button
                              type="submit"
                              className={`w-full py-2 rounded-xl text-[14px] font-extrabold text-white transition ${themeScheme.bg} ${themeScheme.hover} mt-1`}
                            >
