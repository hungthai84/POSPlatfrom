...
export default function Orders({ orders, config, onUpdateOrderStatus }: OrdersProps) {
  ...
  const handleExportCSV = () => {
    const headers = ['ID', 'Khách hàng', 'SĐT', 'Tổng tiền', 'Giảm giá', 'Khách trả', 'Hình thức TT', 'Trạng thái', 'Ngày tạo'];
    const data = filteredOrders.map(o => [
      ...
      o.discount,
      o.finalAmount,
      o.paymentMethod === 'cash' ? 'Tiền mặt' : o.paymentMethod === 'card' ? 'Quẹt thẻ' : o.paymentMethod === 'bank_transfer' ? 'Chuyển khoản' : 'Ví điện tử',
      o.status,
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl border border-slate-200 overflow-hidden slide-in-from-bottom-4 animate-in duration-300">
            ...
            <div className="p-6 space-y-6 overflow-y-auto max-h-[70vh]">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-slate-50 rounded-2xl space-y-1">
                  ...
                  <p className="text-sm font-black text-slate-800">{activeOrder.customer ? activeOrder.customer.name : 'Khách vãng lai'}</p>
                  <p className="text-[11px] text-slate-500 font-medium">{activeOrder.customer ? activeOrder.customer.phone : 'Mua lẻ tại quầy'}</p>
                </div>
                <div className="p-4 bg-slate-50 rounded-2xl space-y-1">
                  <p className="text-[9px] font-bold text-slate-400 uppercase">Trạng thái & Thanh toán</p>
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase ${
                      activeOrder.status === 'completed' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'
                    }`}>
                      {activeOrder.status === 'completed' ? 'Hoàn tất' : 'Đang xử lý'}
                    </span>
                    <span className="text-[10px] font-bold text-slate-600">
                      {activeOrder.paymentMethod === 'cash' ? '💵 TM' : '💳 Thẻ/CK'}
                    </span>
                  </div>
              ...
              <div className="space-y-3">
                <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">Chi tiết hàng hóa ({activeOrder.items.length})</h4>
                <div className="bg-white border border-slate-100 rounded-2xl divide-y divide-slate-50 overflow-hidden">
                  {activeOrder.items.map((it) => (
                    <div key={it.variantId} className="p-4 flex justify-between items-center hover:bg-slate-50 transition-colors">
                      <div className="min-w-0 flex-1">
                        <span className="text-sm font-bold text-slate-800 block truncate">{it.productName}</span>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-[10px] text-slate-400 font-mono">SKU: {it.sku}</span>
                          <span className="w-1 h-1 rounded-full bg-slate-200" />
                          <span className="text-[10px] text-slate-500 font-bold">{formatCurrency(it.price)}</span>
                        </div>
                      </div>
                      <div className="text-right shrink-0 ml-4">
                        <span className="text-sm font-black text-slate-900 block">{formatCurrency(it.price * it.quantity)}</span>
                        <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded">x{it.quantity}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="space-y-3 pt-2">
                <div className="p-4 bg-slate-900 text-white rounded-3xl shadow-xl shadow-slate-200 space-y-3">
                  ...
                  <div className="flex justify-between text-lg font-black pt-2 border-t border-white/10">
                    <span>KHÁCH PHẢI TRẢ</span>
                    <span className="text-blue-400">{formatCurrency(activeOrder.finalAmount)}</span>
                  </div>
                </div>
                {activeOrder.notes && (
                  <div className="p-4 bg-blue-50/50 rounded-2xl border border-blue-100/50">
                    <p className="text-[10px] font-bold text-blue-500 uppercase mb-1 flex items-center gap-1.5">
                      <Info size={12} />
                      Ghi chú đơn hàng
                    </p>
                    <p className="text-xs text-blue-800 font-medium leading-relaxed">{activeOrder.notes}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100 flex gap-3">
              <button
                ...
                className="flex-1 bg-white border border-slate-200 hover:bg-slate-100 text-slate-700 font-bold text-xs py-3 rounded-2xl transition flex items-center justify-center gap-2 shadow-sm"
              >
                <Printer size={16} />
                In Hóa Đơn
              </button>
              {activeOrder.status !== 'completed' && activeOrder.status !== 'canceled' && (
                <button
                  ...
                  className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs py-3 rounded-2xl transition flex items-center justify-center gap-2 shadow-lg shadow-blue-200"
                >
                  <Check size={16} />
                  Hoàn Tất
                </button>
              ...
              {activeOrder.status === 'completed' && (
                 <button
                 onClick={() => setActiveOrder(null)}
                 className="flex-1 bg-slate-800 text-white font-bold text-xs py-3 rounded-2xl transition"
               >
                 Đóng
               </button>
              )}
