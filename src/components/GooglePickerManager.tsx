...
export default function GooglePickerManager({ onLogoSelected }: GooglePickerManagerProps) {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (token) {
        ...
        throw new Error('Không thể nhận Access Token từ tài khoản Google của bạn.');
      }
    } catch (err: any) {
      console.error('Google authorization error:', err);
      // Give a friendly error message
      if (err.code === 'auth/popup-blocked') {
        setError('Popup đăng nhập bị chặn. Vui lòng cho phép quyền mở popup trong trình duyệt.');
      } else {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200/60 p-5 shadow-3xs space-y-4" id="google-picker-integration-card">
      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h4 className="font-extrabold text-slate-800 text-xs uppercase tracking-wide">Phụ đính Google Picker</h4>
            <p className="text-[10px] text-slate-450 font-semibold mt-0.5">Truy xuất tài liệu an toàn từ Google Drive cá nhân</p>
          </div>
        </div>
        ...
        {accessToken ? (
          ...
          <div className="flex items-center gap-1 text-[10px] text-slate-400 font-bold bg-slate-50 px-2 py-0.5 rounded border border-slate-100">
            <CloudOff size={11} />
      ...
      {!accessToken ? (
        <div className="py-6 flex flex-col items-center justify-center text-center space-y-3">
          ...
          <div className="space-y-1">
            <h5 className="font-extrabold text-slate-700 text-xs">Yêu cầu quyền hạn Google Drive</h5>
            <p className="text-[10px] text-slate-450 max-w-xs leading-normal">
              Cấp quyền để lựa chọn ảnh, tài liệu hóa đơn hoặc file dữ liệu nhập kho trực tiếp từ Google Drive của bạn.
            </p>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-450 text-white font-extrabold rounded-lg text-xs transition duration-150 flex items-center gap-2 shadow-xs cursor-pointer"
          >
            {loading ? (
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              { key: 'SPREADSHEETS', name: 'Trang tính', icon: FileSpreadsheet },
            ].map((item) => {
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={loading}
              className="flex-1 py-2.5 px-4 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-405 text-white font-extrabold rounded-lg text-xs transition duration-150 flex items-center justify-center gap-2 shadow-xs cursor-pointer"
            >
              {loading ? (
                <span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />
              ) : (
            ...
            <div className="p-3 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-150/70 rounded-xl space-y-3 animate-in slide-in-from-bottom-2 duration-150 text-left">
              ...
              <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-blue-100/50">
                <a
                  ...
                  className="px-2 py-1 bg-white hover:bg-slate-50 text-slate-650 flex items-center gap-1 rounded border border-slate-200 font-bold text-[10px] transition duration-150 cursor-pointer"
                >
                ...
                {selectedFile.mimeType?.includes('image') && onLogoSelected && (
                  <button
                    ...
                    className="px-2 py-1 bg-emerald-600 hover:bg-emerald-700 text-white flex items-center gap-1 rounded border border-emerald-600 font-bold text-[10px] transition duration-150 cursor-pointer"
                  >
                ...
                <button
                  ...
                  className="px-2 py-1 hover:bg-rose-50 text-rose-600 flex items-center gap-1 rounded font-bold text-[10px] transition duration-150 ml-auto"
                >
                  <Trash2 size={11} />
                  <span>Xóa</span>
                </button>
