...
export default function Returns({
  ...
  const [activeRequest, setActiveRequest] = useState<ReturnRequest | null>(
    null,
  );
  ...
  return (
    <div
      ...
      id="returns-mgmt-screen"
    >
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="space-y-0.5">
          <h2 className="text-[20px] font-extrabold text-slate-800 tracking-tight">
            Hồ Sơ Đổi Trả & Hoàn Tiền
          </h2>
          <p className="text-[14px] text-slate-450">
            Quản lý các yêu cầu trả hàng, lý do trả và xử lý hoàn lại tiền.
          </p>
        </div>
        <button className="flex items-center gap-2 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-bold text-[14px] px-4 py-2.5 rounded-lg shadow-2xs transition">
                  ...
                  filteredReturns.map((req) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td
                          className="py-4 px-4 text-[11px] truncate max-w-[120px]"
                          title={req.reason}
                        >
                          {req.reason}
                        </td>
          ...
          {activeRequest ? (
            <div className="bg-white rounded-xl border border-slate-200/60 p-5 shadow-3xs space-y-4 sticky top-24 animate-fade-in-right">
              ...
              <div className="space-y-2 text-[14px]">
                <p className="font-bold text-slate-700 uppercase text-[11px] tracking-wider mb-1">
                  Mặt hàng trả ({activeRequest.items.length})
                </p>
                <div className="space-y-2 max-h-48 overflow-y-auto pr-1 scrollbar-thin">
                  {activeRequest.items.map((it) => (
                    <div
                      key={it.variantId}
                      className="flex justify-between items-center bg-rose-50/30 p-2 rounded border border-rose-100/50"
                    >
              ...
              <div className="border-t border-slate-100 pt-3 space-y-2 text-[14px] text-slate-600">
                <div className="flex justify-between text-[16px] font-black text-rose-600 pb-2">
                  <span>Tổng tiền bồi hoàn:</span>
                  <span>{formatCurrency(activeRequest.refundAmount)}</span>
                </div>
                <div className="bg-slate-50 rounded p-2 text-[12px] text-slate-700 border border-slate-100 italic">
                  <strong>Lý do:</strong> {activeRequest.reason}
                </div>
                {activeRequest.notes && (
                  <div className="bg-indigo-50/50 rounded p-2 text-[12px] text-indigo-800 border border-indigo-100/50">
              ...
              {activeRequest.status === "pending" && onUpdateReturnStatus && (
                <div className="pt-2 border-t border-slate-100 space-y-2">
                  <button
                    ...
                    className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold text-[14px] py-2 rounded-lg transition"
                  >
                  ...
                  <button
                    ...
                    className="w-full bg-red-50 hover:bg-red-100 text-red-600 font-bold text-[14px] py-2 rounded-lg transition border border-red-200"
                  >
                    Từ Chối Yêu Cầu
                  </button>
                </div>
              ...
              {activeRequest.status === "approved" && onUpdateReturnStatus && (
                <div className="pt-2 border-t border-slate-100 space-y-2">
                  <button
                    ...
                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[14px] py-2 rounded-lg transition"
                  >
            ...
            <div className="bg-white rounded-xl border border-slate-200/60 p-10 text-center text-slate-400 font-semibold text-[14px] h-64 flex flex-col items-center justify-center sticky top-24">
